...
class aerial(boardElements):
    def __init__(self , background):
        ...
        self.health = 40
        self.huts_present = 0
        self.prev_block = space = Back.GREEN + Fore.GREEN + ' ' + Fore.RESET + Back.RESET
        self.prev_stepx = 0
        self.prev_stepy = 0
    def setMatrix(self , x , y): 
        hyph = Back.RED + Fore.BLACK + '-' + Fore.RESET + Back.RESET
        alpha_b = Back.YELLOW + Fore.BLACK + 'A' + Fore.RESET + Back.RESET
    ...
    def setMatrix2(self , x , y): 
        hyph = Back.RED + Fore.BLACK + '-' + Fore.RESET + Back.RESET
        alpha_b = Back.WHITE + Fore.BLACK + 'A' + Fore.RESET + Back.RESET
        slash = Back.RED + Fore.BLACK + '/' + Fore.RESET + Back.RESET
        space = Back.RED + Fore.BLACK + ' ' + Fore.RESET + Back.RESET
        bslash = Back.RED + Fore.BLACK + '\\' + Fore.RESET + Back.RESET
        straight = Back.RED + Fore.BLACK + '|' + Fore.RESET + Back.RESET
    ...
    def setMatrix3(self , x , y): 
        hyph = Back.RED + Fore.BLACK + '-' + Fore.RESET + Back.RESET
        alpha_b = Back.RED + Fore.BLACK + 'A' + Fore.RESET + Back.RESET
        slash = Back.RED + Fore.BLACK + '/' + Fore.RESET + Back.RESET
        space = Back.RED + Fore.BLACK + ' ' + Fore.RESET + Back.RESET
        bslash = Back.RED + Fore.BLACK + '\\' + Fore.RESET + Back.RESET
        straight = Back.RED + Fore.BLACK + '|' + Fore.RESET + Back.RESET
    ...
    def moveandkill(self , all_huts , all_canons , all_wizards , background , hall , king):
        ...
        canon_check = 0
        wizard_check =0
        canon_c = canon(background)
        wizard = wizard_tower(background)
        defense_finished = 1
        all_defense = []
        for defense in all_canons:
            all_defense.append(defense)
        for defense in all_wizards:
            all_defense.append(defense)            
        for defense in all_defense:
            if defense.health>0:
                ...
                targety = defense.y  
                attack_defense = 1 
                if defense.name == "canon":
                    canon_c = defense
                    canon_check = 1
                    #print("attacking canon")
                if defense.name == "wizard_tower":
                    wizard = defense  
                    wizard_check = 1
                    #print("attacking wizard tower")  
                break     
        if defense_finished == 1:
                for i in range(6):
        ...
        if(abs(self.x - targetx )>2):         
            if(self.x > targetx):
                if self.prev_block == Back.YELLOW + Fore.BLACK + 'A' + Fore.RESET + Back.RESET or self.prev_block == Back.WHITE + Fore.BLACK + 'A' + Fore.RESET + Back.RESET or self.prev_block == Back.RED + Fore.BLACK + 'A' + Fore.RESET + Back.RESET:
                    self.prev_block = Back.GREEN + Fore.GREEN + ' ' + Fore.RESET + Back.RESET
                background._grid[self.x][self.y] = self.prev_block
                self.setMatrix(self.x-1 , self.y)
                self.prev_block = background._grid[self.x][self.y]
                self.prev_stepx = self.x
                self.prev_stepy = self.y
                #self.x = self.x -1
                self.inpos = 0
        ...
        if (self.inpos==1 and self.freq%10==0):
                for i in range(6):
                    if (attack_defense == 1 and (abs(self.x - targetx) <=2  or abs(self.y - targety) <=2 )):
                        if canon_check==1 and wizard_check==0:
                            self.destroycanon(canon_c , background)
                        if canon_check==0 and wizard_check==1:
                            self.destroywizard(wizard , background)                            

                    if (target ==1 and abs(73 - self.y)<2) :
                        self.destroyhall(hall , background)
                        if hall.health <=0:
                            background._grid[self.prev_stepx][self.prev_stepy] = Back.GREEN + Fore.GREEN + ' ' + Fore.RESET + Back.RESET
                            background._grid[self.x][self.y] = Back.GREEN + Fore.GREEN + ' ' + Fore.RESET + Back.RESET
                            hall.setMatrix4()
    ...
    def destroycanon(self , canon , background):
        canon.health = canon.health - self.damage
        if(canon.health>60 and canon.health<=100):
            canon.setMatrix(canon.x , canon.y)
            background.placeElement(canon)               
    ...
    def destroywizard(self , canon , background):
        canon.health = canon.health - self.damage
        if(canon.health>60 and canon.health<=100):
            canon.setMatrix(canon.x , canon.y)
            background.placeElement(canon)               
